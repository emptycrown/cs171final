...
function startGame(){
    ...
    enemyGeneration = setInterval(function(){
        ...
        svg.selectAll(".empty").data(e).enter().append("circle")
            ...
            .attr("fill",function(d,i){ return color(d.type);} )
            .attr("AT",5000) //set at time tracker for game pause;
            .transition().duration(5000).delay(function(d,i){ return d.delay(d,i); })
            .tween("assignment", tickTween)
            .attr("cx", width/2)
            .attr("cy", height/2)
            .remove();
...
function updateScore(score){
    svg.select("text").transition().duration(500).tween("text", function() {
    ...
    svg.select(".indicator").transition().duration(500)
        .attrTween("d", function(){
            ...
            d3.select(".score").text(Math.floor(game_status.score));
        });
...
function setup(){
    ...
    svg = d3.select("#game")
        ...
        .attr("width", width)
        .attr("height", height)
        .attr("style","background-color:#212121");
